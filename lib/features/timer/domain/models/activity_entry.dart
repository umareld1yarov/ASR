...
class ActivityEntry {
  ...
  /// Стабильный глобальный идентификатор для синхронизации между устройствами.
  /// Локальный Isar [id] никогда не отправляется как идентичность записи.
  @Index()
  String syncId = '';

  /// Момент последнего содержательного изменения (epoch millis).
  /// Обычная синхронизация это поле не обновляет.
  int updatedAt = 0;
  ...
  /// Облачные фото, которые пользователь удалил офлайн или при временной
  /// ошибке сети. Очищается только после подтверждённого удаления из Storage.
