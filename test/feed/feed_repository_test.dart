...
void main() {
  ...
  group('FeedRepository', () {
    ...
    test('getEntriesByDate возвращает только записи за запрошенный день', () async {
      await insertEntry(
        name: 'День 1',
        categoryKey: 'work',
        startedAt: 1000,
    ...
    test('getEntriesByDate сортирует записи по времени начала (startedAt)', () async {
      await insertEntry(
    ...
    test('getEarliestStartedAt возвращает самое раннее время начала среди неудалённых записей', () async {
    ...
    test('updateEntry обновляет имя, категорию, заметки, mood, obstacles и nextExperiment', () async {
      ...
      expect(updated.nextExperiment, equals('Фокусироваться на одной задаче'));
      expect(updated.updatedAt, isPositive);
    });

    test('updateEntry не изменяет защищённые временные границы (startedAt, endedAt, durationSeconds)', () async {
      final entry = await insertEntry(
        name: 'Задача',
        categoryKey: 'work',
        startedAt: 10000,
    ...
    test('deleteEntry устанавливает isDeleted в true и обновляет updatedAt', () async {
      final entry = await insertEntry(
        name: 'К удалению',
        categoryKey: 'waste',
        startedAt: 1000,
    ...
    test('addPhoto и removePhoto корректно управляют списком путей к фото', () async {
      final entry = await insertEntry(
        name: 'С фото',
        categoryKey: 'family',
        startedAt: 1000,
    ...
    test('getRandomEntryWithPhoto и getAllEntriesWithPhotos выбирают только неудалённые записи с фото', () async {
    ...
    test('getDayStoryGroups группирует максимум 2 записи на категорию в хронологическом порядке появления', () async {
